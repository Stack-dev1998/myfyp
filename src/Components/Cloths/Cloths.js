...
export default class Foods extends Component {
  ...
  componentDidMount() {
    axios({
      method: "post",
      url: "http://localhost:5000/public/cloths",
      data: {
  ...
  loadMoreHandler() {
    axios({
      method: "post",
      url: "http://localhost:5000/public/cloths/load-more",
      data: {
        pageNumber: this.state.pageNumber,
        pageSize: this.state.pageSize,
      },
      ...
      var cloths = this.state.clothData;
      res.data.map((item) => {
        cloths.push(item);
      });
      this.setState({
  ...
  searchHandler() {
    if (this.state.searchValue === null || this.state.searchValue === "") {
      ...
      axios({
        method: "post",
        url: "http://localhost:5000/public/cloths/search",
  ...
  onChangeHandler = (name) => {
    ...
    if (name === "") {
      this.setState({
        clothData: this.state.originalData,
      });
    }
  };
        ...
        <Row>
          ...
          {this.state.isLoaded ? (
            <Col xs="12" sm="9" md="9" lg="9">
              <Row>
                {this.state.clothData
                  ? this.state.clothData.map((item) => {
                      return (
                        <Col
                          xs="12"
                          sm="6"
